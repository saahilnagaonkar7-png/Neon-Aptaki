...
// Three.js Core
let scene, camera, renderer, composer, sunLight;
let clock;
let floor;
let pillars = [];
let outerBarriers = [];
...
const dom = {
    ...
    btnGraphics: document.getElementById('btn-graphics')
};
...
function init() {
    ...
    // Renderer (conditional anti-aliasing & shadows to support integrated graphics)
    renderer = new THREE.WebGLRenderer({ 
        antialias: highGraphics, 
    ...
    renderer.toneMappingExposure = highGraphics ? 1.15 : 1.35; // slightly higher exposure on low-end
    document.getElementById('game-container').appendChild(renderer.domElement);

    // Post-processing Composer for Bloom (only in High Graphics mode)
    if (highGraphics) {
...
function setupLights() {
    ...
    // Main dim purple key light (cast shadows only in High Graphics)
    sunLight = new THREE.DirectionalLight(0x5500aa, 0.8);
    sunLight.position.set(25, 45, 25);
    sunLight.castShadow = highGraphics;
    if (highGraphics) {
        sunLight.shadow.mapSize.width = 512; // Optimized from 1024 to support i3 GPUs
...
function createEnvironment() {
    ...
    // Repeat grid texture many times to create a massive endless plane
    ...
    floor.receiveShadow = highGraphics;
    scene.add(floor);

    // Spawn Obstacle Pillars inside arena with multi-colored neon rings & lights (Well-lighted slate blue-grey base)
    ...
    obstaclePillarsData.forEach((p, index) => {
        ...
        // Base cylinder (Lighter slate blue-grey for visibility/lighting absorption)
        const geo = new THREE.CylinderGeometry(p.r, p.r, 12, 16);
        const mat = new THREE.MeshStandardMaterial({
            color: 0x22223a, // Lighter, light-absorbing slate grey
            roughness: 0.35,
            metalness: 0.25
        });
        const pillar = new THREE.Mesh(geo, mat);
        pillar.position.set(p.x, 6, p.z);
        pillar.castShadow = highGraphics;
        pillar.receiveShadow = highGraphics;
        scene.add(pillar);

        // Multi-colored glowing energy rings - added directly to the pillar mesh
        ...
        ring1.position.set(0, -3.0, 0); // local Y relative to pillar center (Y=6)
        ring1.rotation.x = Math.PI/2;
        pillar.add(ring1);

        const ring2 = new THREE.Mesh(ringGeo, ringMat);
        ring2.position.set(0, 2.0, 0); // local Y
        ring2.rotation.x = Math.PI/2;
        pillar.add(ring2);

        // DOUBLE neon lights per pillar (added as children, scales down on Low graphics)
        if (highGraphics) {
            const baseLight = new THREE.PointLight(ringColor, 3.8, 12.0);
            baseLight.position.set(0, -5.5, 0); // local base
            pillar.add(baseLight);
        }

        const midLight = new THREE.PointLight(ringColor, highGraphics ? 3.2 : 2.0, 10.0);
        midLight.position.set(0, 0, 0); // local center
        pillar.add(midLight);

        // Save reference for collision detection & wrapping
        pillar.userData.radius = p.r;
        pillar.userData.initialX = p.x;
        pillar.userData.initialZ = p.z;
        pillar.userData.isMonolith = false;
        pillars.push(pillar);
    });
    ...
    monolith.castShadow = highGraphics;
    monolith.receiveShadow = highGraphics;
    ...
    const centerBeaconLight = new THREE.PointLight(0xff0055, highGraphics ? 5.0 : 3.0, 18.0);
    ...
    collisionMesh.userData.radius = 2.0;
    collisionMesh.userData.isMonolith = true;
...
function setupUIListeners() {
    ...
    // Graphics settings toggle click handler
    dom.btnGraphics.addEventListener('click', () => {
        highGraphics = !highGraphics;
        if (highGraphics) {
            dom.btnGraphics.textContent = "GRAPHICS: HIGH (BLOOM)";
            dom.btnGraphics.classList.remove('low-perf');
        } else {
            dom.btnGraphics.textContent = "GRAPHICS: LOW (PERFORMANCE)";
            dom.btnGraphics.classList.add('low-perf');
        }

        // Apply graphics settings immediately to renderer & shadows
        if (renderer) {
            renderer.shadowMap.enabled = highGraphics;
            renderer.setPixelRatio(highGraphics ? Math.min(window.devicePixelRatio, 2) : 1);
            renderer.toneMappingExposure = highGraphics ? 1.15 : 1.35;
        ...
        // Dynamically toggle base lights of pillars to save GPU fill rate
        pillars.forEach(pillar => {
            pillar.children.forEach(child => {
                if (child.isPointLight && Math.abs(child.position.y - (-5.5)) < 0.1) {
                    child.visible = highGraphics;
                }
            });
        });
    });
...
function checkCollisions(newPos) {
    // No boundaries (Infinite map!)

    // Obstacle pillars dynamic check (reads from live mesh positions)
    const playerRadius = player.radius;
    pillars.forEach(pillar => {
        const px = pillar.position.x;
        const pz = pillar.position.z;
        const pr = pillar.userData.radius || 1.5;

        const dx = newPos.x - px;
        const dz = newPos.z - pz;
        const dist = Math.hypot(dx, dz);
        const minDist = pr + playerRadius;

        if (dist < minDist) {
            // Push player out along collision angle
            const angle = Math.atan2(dz, dx);
            newPos.x = px + Math.cos(angle) * minDist;
            newPos.z = pz + Math.sin(angle) * minDist;
        }
    });
...
function updateGame(dt) {
    ...
    // Dynamic Pillar Wrapping (Endless Map obstacle field!)
    pillars.forEach(pillar => {
        if (pillar.userData.isMonolith) return;

        let dx = pillar.position.x - camera.position.x;
        let dz = pillar.position.z - camera.position.z;
        
        const wrapRange = 85;
        const doubleRange = wrapRange * 2;

        if (dx > wrapRange) {
            pillar.position.x -= doubleRange;
        } else if (dx < -wrapRange) {
            pillar.position.x += doubleRange;
        }

        if (dz > wrapRange) {
            pillar.position.z -= doubleRange;
...
function animate() {
    ...
    // Render step (use EffectComposer for Bloom in High Graphics, default WebGLRenderer in Low)
    if (highGraphics && composer) {
        composer.render();
    } else {
...
function resetGame() {
    ...
    // Reset pillars to their starting grid positions
    pillars.forEach(pillar => {
        if (pillar.userData.isMonolith) return;
        pillar.position.x = pillar.userData.initialX;
        pillar.position.z = pillar.userData.initialZ;
    });
